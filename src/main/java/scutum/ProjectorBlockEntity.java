...
public class ProjectorBlockEntity extends BlockEntity {
    ...
    public static final double BASIC_TIER_MULTIPLIER = 1.23;
    public static final double ADVANCED_TIER_MULTIPLIER = 0.85;

    public static final double BASIC_ENERGY_MULTIPLIER = 0.05;
    public static final double ADVANCED_ENERGY_MULTIPLIER = 0.05;
    ...
    private void createField(ServerWorld world) {
        fieldBlocks.clear(); BlockPos center = this.pos; Block ff = ProjectorMod.FORCEFIELD_BLOCK;
        if (isAdvanced()) {
            final int r = getRadius(); final double thickness = 1.0; final double halfThickness = thickness / 2.0;
            List<List<int[]>> layerOffsets = new ArrayList<>(r + 1); for (int i = 0; i <= r; i++) layerOffsets.add(new ArrayList<>());
            double baseR = r; int maxXZBase = (int) Math.ceil(baseR + 0.5);
            for (int dx = -maxXZBase; dx <= maxXZBase; dx++) for (int dz = -maxXZBase; dz <= maxXZBase; dz++) {
                double distXZ = Math.sqrt(dx * dx + dz * dz); if (distXZ <= baseR) { if (dx == 0 && dz == 0) continue; layerOffsets.get(0).add(new int[]{dx, dz}); }
            }
            double targetR1 = Math.sqrt(Math.max(0, r * r - 1 * 1)); int maxXZ1 = (int) Math.ceil(targetR1 + 0.5);
            List<int[]> ring1 = new ArrayList<>(); for (int dx = -maxXZ1; dx <= maxXZ1; dx++) for (int dz = -maxXZ1; dz <= maxXZ1; dz++) {
                double distXZ = Math.sqrt(dx * dx + dz * dz);
                if (Math.abs(distXZ - targetR1) <= halfThickness) ring1.add(new int[]{dx, dz});
            }
            layerOffsets.set(1, ring1); layerOffsets.set(2, new ArrayList<>());
            for (int[] o : ring1) layerOffsets.get(2).add(new int[]{o[0], o[1]});
            for (int dy = 3; dy <= r; dy++) {
                double targetR = Math.sqrt(Math.max(0, r * r - dy * dy)); int maxXZ = (int) Math.ceil(targetR + 0.5);
                List<int[]> ring = new ArrayList<>(); for (int dx = -maxXZ; dx <= maxXZ; dx++) for (int dz = -maxXZ; dz <= maxXZ; dz++) {
                    double distXZ = Math.sqrt(dx * dx + dz * dz);
        ...
        // basic branch
        {
            ...
            for (int dy = 0; dy <= primitiveMaxDy; dy++) {
                ...
                for (int dx = -maxXZ; dx <= maxXZ; dx++) for (int dz = -maxXZ; dz <= maxXZ; dz++) {
                    double distXZ = Math.sqrt(dx * dx + dz * dz); boolean place; if (dy == 0) { place = distXZ <= targetR; if (dx == 0 && dz == 0) place = false; }
    ...
    public void serverTick(ServerWorld world) {
        if (energy < MAX_ENERGY) receiveEnergyFromAdjacent(); if (active) { int cost = energyCostPerTick > 0 ? energyCostPerTick : 1; if (energy >= cost) { energy -= cost; applyFieldEffects(world); markDirty(); } else { removeField(world); active = false; world.setBlockState(pos, getCachedState().with(ProjectorBlock.LIT, false), 3); markDirty(); } }
    }
    ...
    private void applyFieldEffects(ServerWorld world) {
        ...
        for (Entity e : entities) {
            double dx = e.getX() - (pos.getX() + 0.5); double dy = e.getY() - (pos.getY() + 0.5); double dz = e.getZ() - (pos.getZ() + 0.5);
    ...
    public void applyFieldDamage(int amount) {
        if (amount <= 0) return; fieldHealth -= amount; if (fieldHealth <= 0) {
            if (this.world instanceof ServerWorld sw) {
                ...
                sw.playSound(null, pos, SoundEvents.ENTITY_ITEM_BREAK, SoundCategory.BLOCKS, 1.0f, 1.0f);
                try { int rawId = Block.getRawIdFromState(oldState); sw.syncWorldEvent(2001, pos, rawId); } catch (Exception ignored) {}
            }
        }
        markDirty();
    }
